...
const LoginForm = ({ onLogin }) => {
  ...
  const handleFirstSignIn = async () => {
    ...
    try {
      ...
      setCurrentStep(2);
      setTimeout(() => {
        const passwordInput = document.querySelector('input[type="password"], input[placeholder="Enter Password"]');
        if (passwordInput) passwordInput.focus();
      }, 600);
    } catch (error) {
      const errorMessage = error.response?.data?.error || error.response?.data?.message || error.message || "User validation failed";
      if (errorMessage.toLowerCase().includes("user not found") || errorMessage.toLowerCase().includes("invalid user") || error.response?.status === 404) {
...
console.log("Dispatched user data to Redux:", cleanData);
      ...
      } else if (errorMessage.toLowerCase().includes('user') || errorMessage.toLowerCase().includes('email') || errorMessage.toLowerCase().includes('username') || errorMessage.toLowerCase().includes('not found')) {
        setCurrentStep(1);
        setErrors({ userIdAlias: 'User not found. Please check your UserID, Alias, or Email.' });
        ...
        <div className="relative w-full max-w-3xl mx-auto overflow-hidden">
          ...
          <div className="relative bg-white shadow-2xl overflow-hidden min-h-[400px] sm:min-h-[500px] flex flex-col lg:flex-row">
            <div className={`${isMobile ? 'order-1' : 'order-2'} flex-1 relative bg-gradient-white`}>
              ...
              <div className={`absolute inset-0 p-2 sm:p-6 lg:p-10 flex flex-col justify-center transition-transform duration-500 ease-in-out ${currentStep === 2 ? 'translate-x-0' : 'translate-x-full'}`}>
