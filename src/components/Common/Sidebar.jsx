...
const Sidebar = ({
  ...
  useEffect(() => {
    ...
    if (role) {
      setDynamicRoles([role.toLowerCase()]);
    } else {
      setDynamicRoles([]);
    }
  }, [auth.user]);

  const normalizedRoles = dynamicRoles.length ? dynamicRoles : ["admin", "employee"];

  const userRole = userProfile?.role?.toLowerCase() || normalizedRoles[0];
  ...
  const sidebarSections = [
    ...
    },
    {
      ...
      icon: <User size={20} />,
      route: "/ProfilePage",
      roles: normalizedRoles.includes("security guard") ? normalizedRoles : [...normalizedRoles, "security guard"]
    },
    {
      id: "organization",
      title: "Organization",
      ...
      route: "/Organization",
      roles: normalizedRoles.includes("admin") ? ["admin"] : []
    },
    {
      id: "roles",
      title: "Roles",
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-screen bg-slate-800 text-white flex flex-col shadow-lg transition-transform duration-300 z-20 ${isOpen ? 'translate-x-0' : '-translate-x-full'} ${className}`}
      style={{ width: "200px" }}
    >
      <div className="flex justify-center p-4 text-center border-b border-slate-700">
        <div className="space-x-3">
          <div>
            <h1 className="text-lg font-bold">GATE CHECK</h1>
